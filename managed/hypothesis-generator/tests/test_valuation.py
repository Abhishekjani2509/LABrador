...
@pytest.fixture
def slate(graph: KnowledgeGraph) -> Slate:
...
def test_no_emitted_evidence_key_can_clear_a_labrador_gate(slate: Slate, frame: ProgramFrame):
...
def test_nothing_emitted_is_ever_marked_synthetic(slate: Slate, frame: ProgramFrame):
    ...
    blob = json.dumps([p for p in emit(slate, frame).programs])
...
def test_analogical_transfer_yields_no_mechanism_nodes(slate: Slate):
...
def test_chain_mechanism_nodes_exclude_the_endpoint(slate: Slate):
    chains = [h for h in slate.hypotheses if h.motif == "transitive_chain"]
    assert chains
...
def test_donor_caveat_is_carried_onto_the_program(slate: Slate, frame: ProgramFrame):
    for program in emit(slate, frame).programs:
        for indication in [program["initial_indication"], *program["expansion_indications"]]:
...
def test_population_and_access_are_emitted_empty(slate: Slate, frame: ProgramFrame):
...
def test_no_development_path_or_price_is_invented(slate: Slate, frame: ProgramFrame):
    for program in emit(slate, frame).programs:
...
def test_protein_subject_needs_an_explicit_modality(slate: Slate, frame: ProgramFrame):
    ...
    hypothesis = next(h for h in slate.hypotheses).model_copy(deep=True)
    hypothesis.evidence["things"][hypothesis.subject]["kind"] = "protein"
    stub = Slate(graph_id="g", round=1, question="q", hypotheses=[hypothesis])
...
def test_one_molecule_becomes_one_program_with_two_labels(slate: Slate, frame: ProgramFrame):
    ...
    assert multi, "pirfenidone appears twice in the repurposing slate"
    program = multi[0]
...
def test_extra_labels_are_reported_not_dropped(frame: ProgramFrame, slate: Slate):
    """No silent caps: LABrador models one expansion, so a third label has to be
    said out loud rather than quietly discarded."""
    subject_hypotheses = [h for h in slate.hypotheses if h.subject == "t1"]
    ...
    stub = Slate(
        graph_id="g", round=1, question="q", hypotheses=[*subject_hypotheses, *padded]
    )
    reasons = {s.reason for s in emit(stub, frame).skipped}
    assert "labrador_two_label_limit" in reasons


def test_non_disease_endpoints_are_skipped_with_a_reason(slate: Slate, frame: ProgramFrame):
    hypothesis = next(h for h in slate.hypotheses).model_copy(deep=True)
...
def test_empty_slate_says_so_rather_than_returning_nothing(frame: ProgramFrame):
    emission = emit(Slate(graph_id="g", round=1, question="q"), frame)
...
def test_emission_validates_against_labradors_own_contracts(slate: Slate, frame: ProgramFrame):
