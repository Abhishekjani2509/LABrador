...
from hyp_gen.scoring import score_all
from hyp_gen.select import pareto_front, select, similarity
...
def test_mmr_reduces_redundancy(index: GraphIndex) -> None:
    ...
    exists to fix, so the diverse slate must overlap less."""
...
def test_quota_block_frees_the_slot_for_a_different_idea(index: GraphIndex) -> None:
    """A rejected pick must not shrink the slate -- the next-best *different*
