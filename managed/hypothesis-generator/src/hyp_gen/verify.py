...
from hyp_gen import validate
from hyp_gen.candidates import Candidate
from hyp_gen.evidence import EvidencePack
...
def _worst(
    ...
    the slate at all; gate status decides what the verification table says. A
...
def gate_independence(ctx: GateContext) -> GateResult:
    ...
    one result, and it is the single most common way a slate looks better
...
def gate_adversarial(ctx: GateContext) -> GateResult:
    ...
    """
    from hyp_gen import reason  # local: keeps verify.py importable without the model stack
