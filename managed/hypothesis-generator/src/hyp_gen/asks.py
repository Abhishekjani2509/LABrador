"""Turn each hypothesis's weakest point into a Stage 1 request.
...
by id rather than prose, so Stage 1 can act on them without interpretation.
...
from hyp_gen.candidates import Candidate
from hyp_gen.graph import GraphIndex
from hyp_gen.params import Params
from hyp_gen.schema import Ask
from hyp_gen.scoring import Scores
...
def asks_for(
    ...
    # Only worth a round trip for hypotheses near the top of the slate: a gap
...
def dedupe(asks: list[Ask]) -> list[Ask]:
    """One ask per target. Stage 1 takes one ask per request anyway, and the
