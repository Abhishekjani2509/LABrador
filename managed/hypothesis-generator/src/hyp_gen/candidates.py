...
``gap_closure``
    Stage 1 noticed a pair that its own links imply but nobody states. The
...
class Candidate:
    ...
    def key(self) -> tuple:
        ...
        slate from being half mirror images."""
...
def _gap_closures(index: GraphIndex, params: Params) -> list[Candidate]:
    ...
    for gap in index.graph.gaps:
        ...
        if params.motifs.require_unstated and index.links_between(a, b):
            ...
            # promoted it (which is `test_gap` working as designed) or Stage 1
