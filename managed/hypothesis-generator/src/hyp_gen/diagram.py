"""Slate → a static SVG of the traces the deterministic half walked.
...
in a page or a markdown file unchanged. It is a pure function of the slate like
every other view here, so the picture can be regenerated from a saved
``slate.json`` without re-running anything.
...
``no`` and ``no_effect`` -- and Stage 1 derives ``state`` from them. So:
...
def _collect(slate: Slate) -> tuple[dict[str, _Node], list[_Edge]]:
...
def _order_rows(nodes: dict[str, _Node], edges: list[_Edge], sweeps: int = 6) -> None:
    ...
    First-seen order is what a reader gets otherwise, and on the demo slate it
...
def _free_label_y(
    ...
    same pixel and overprint into gibberish -- on the demo slate ``L1 inhibits``
...
def to_svg(slate: Slate) -> str:
    """One SVG for the whole slate. Deterministic: same slate, same bytes."""
    nodes, edges = _collect(slate)
    if not nodes:
        nodes = {}
    ...
    out.append(
        ...
        f'fill="{_INK}">{escape(slate.graph_id)} · round {slate.round} · '
        f"{len(slate.hypotheses)} trace"
        f'{"" if len(slate.hypotheses) == 1 else "s"}</text>'
    )
    if slate.question:
        out.append(
            f'<text x="{_PAD}" y="{_PAD + 27}" font-size="12" fill="{_MUTED}">'
            f"{escape(_wrap(slate.question, width - _PAD * 2)[0])}</text>"
        )
    ...
    for edge in edges:
        ...
        if not (a and b):
            ...
            raise ValueError(f"edge {edge.link} has an endpoint not in the slate")
