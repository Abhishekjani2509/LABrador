"""Slate → web UI payload.

The second adapter in this package, next to ``valuation.py``: a pure function
of one ``Slate`` producing the JSON a downstream consumer binds to. There the
consumer is LABrador; here it is a web UI rendering one card per hypothesis.
Being pure over the slate means the payload can be produced — or reproduced —
from a saved ``slate.json`` without re-running the pipeline, exactly like the
report modes.
...
- ``highlights`` — short one-liners saying how the graph supports,
  ...
  from structured slate fields and carries the ids it was assembled from in
  ``refs``**, so the UI can link each line to its evidence and a reader can
  check it. No highlight states anything the slate does not; free prose from
...
from hyp_gen.report import _clip, _failure_badges, _flags, _halt_note
...
class WebPayload(BaseModel):
    ...
    warnings: list[str] = Field(
        default_factory=list,
        description="Slate-level facts every card must be read next to.",
    )
...
def _novelty_highlight(hypothesis: Hypothesis) -> Highlight:
    ...
    A Stage 1 gap note is preferred when present: it is the recorded sentence
    ...
    by_motif = {
        ...
        "gap_closure": (
            f"Stage 1 flagged {subject} ↔ {obj} as implied by its links "
...
def emit(slate: Slate) -> WebPayload:
    """One payload per slate. Pure: same slate in, same payload out."""
    warnings: list[str] = []
    cov = slate.coverage
    ...
    if shared:
        ...
        warnings.extend(
            _clip(c, _LINE)
            for c in slate.hypotheses[0].caveats
            if c in shared
        )
    ...
    return WebPayload(
        ...
        coverage=dict(cov),
        warnings=warnings,
        hypotheses=[_card(h, shared) for h in slate.hypotheses],
    )
