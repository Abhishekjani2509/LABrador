...
`anomalies` array, the estimated `avgDealSizeUsd`, and an `incidentImpact`
...
revenue value of that week's churned customers). Only full weeks matter for
the report — if the CSV's last week has fewer than 7 days, treat it as
in-progress and report on the last full week instead, noting the partial
week separately if it's still worth watching.
