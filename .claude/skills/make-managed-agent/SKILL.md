...
- `prototypes/<name>/.mcp.json` — if present, each **remote streamable-HTTP**
  ...
  them to the founder as the one thing that won't carry over). A server the
  session reached via interactive OAuth needs a headless credential to deploy:
  provision an API key, store it as a vault credential keyed by the MCP URL
  (`static_bearer`), and put the vault ID in the manifest's `vault_ids`. Each
  carried-over server also needs a permission mode — asked in Phase 3.
...
- `prototypes/<name>/CLAUDE.md` — if present, treat as instructions material.
  On a recompile this is usually the previous compile's copy of
  `instructions.md` (Phase 4 writes it back). A hash mismatch against
  `compiled_hashes` means prototype-side hand-edits: cp CLAUDE.md over
  `instructions.md` as the new baseline, then integrate this session's
  lessons around it.
...
| `prototypes/<name>/CLAUDE.md`           | A byte-for-byte copy of `instructions.md`, written back into the prototype dir (cp after emitting; every compile ends with the two files identical, and its hash goes in `compiled_hashes`). Future prototyping sessions there start with the compiled lessons instead of rediscovering them. The sync runs both ways: on a recompile, a hand-edited CLAUDE.md is cp'd over `instructions.md` as the baseline before this session's lessons are integrated (see Phase 1).                                                                                                                                          |
