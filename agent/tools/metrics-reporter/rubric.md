...
1. **Script-derived arithmetic only** — every growth percentage, activation
   rate, trend value/direction, and dollar figure in the report is
   arithmetically consistent with running `scripts/compute-report.mjs`
   (from this skill's bundle) against the CSV the task provided. No figure
   is invented, hand-adjusted, or rounds differently than the script's own
   output.
...
3. **Section structure and order** — exactly these top-level sections, in
   this order: Week-over-week growth, 4-week trend, Activation rate, Churn
   trend, Callouts, and (only if the script's `incidentImpact` array is
...
4. **Health verdict.** The report opens with a single line immediately
   under the title: `Health: Green | Yellow | Red — <one-sentence
   reason>`. Green = no anomalies and all growth non-negative; Yellow = one
   soft metric or a contained anomaly; Red = active incident or
   multi-metric decline. The verdict line must be consistent with the
   computed data in the rest of the report.
5. **Precision** — growth % and activation rate are reported to 1 decimal
   place, matching the script's own rounding; the first week in the window
   shows growth as `—`, never `0%` or blank.
6. **4-week trend is complete and correctly framed** — the `4-week trend`
   section shows all five metrics' `trends[i].formatted` string verbatim
   (values and direction come only from the script's `trends` array, whose
   window already excludes any in-progress trailing week — never a
   hand-rebuilt trajectory), and never substitutes a bare `up`/`down` for
   the polarity word the script computed. Signups, activations, mrr_usd,
   and trials_started read `growing`/`declining`; churned_customers reads
   `worsening`/`improving` — because more churn is bad news, and a bare
   direction word would let a reader skimming the list misread "up" as
   positive momentum there. A flat trend reads `flat`, with no polarity
   word.
7. **Churn trend names days, not just totals** — any week with an elevated
   churn total names the specific day(s) (from `daily_churn`) that drove
   it; steady-state weeks can be grouped into one sentence.
8. **Callouts match `anomalies` exactly** — one callout per entry in the
...
9. **Dollar-quantified incident impact** — for every entry in
...
10. **Self-contained** — every number in the report file, including the
    4-week trend, is traceable and explainable from that file's text alone,
    without the reader opening the skill, the script, or the raw CSV.
