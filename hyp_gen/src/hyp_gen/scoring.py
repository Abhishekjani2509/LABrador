...
Stage 1 ships a `links.confidence` block, and schema note 3 explicitly invites
Stage 2 to recompute it with its own weights. We do, for two reasons: the
...
class LinkSupport:
    ...
    def drift(self) -> float:
        """Ours minus Stage 1's. Large drift either way is worth a look."""
...
def score_candidate(index: GraphIndex, candidate: Candidate, params: Params) -> Scores:
    ...
    if candidate.gap_id:
        ...
        if gap is not None:
            # Mirror Stage 1's own cap: a gap is a proposal, not a finding, so
