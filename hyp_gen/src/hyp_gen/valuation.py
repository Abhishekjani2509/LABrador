"""Slate → LABrador `ProgramInput`. The handoff to the valuation stage.
...
class Skipped(BaseModel):
    ...
    Recorded rather than dropped. A slate of eight that yields two programs has
...
def _indication(
    ...
    *,
    ...
    accessed = _accessed_at(slate)
    evidence: dict[str, Any] = {
        f"hypothesis:{hypothesis.id}": {
            **unsupported(""),
            "source_id": f"{slate.graph_id}@round{slate.round}",
            "evidence_type": "SECONDARY_RESEARCH",
            "grade": "LOW",
...
def program_input(
    ...
    expansions = [
        _indication(h, slate, frame, launch_year=expansion_launch, role="expansion")
        for h in rest[:1]
    ]

    return {
        "program_id": f"{slate.graph_id}-{_slug(initial.subject_name)}",
        ...
        "initial_indication": _indication(
            initial, slate, frame, launch_year=frame.launch_year, role="initial"
        ),
        ...
        "assumptions": {
            ...
            "graph_question": slate.question,
            "graph_coverage": slate.coverage,
            "absence_warning": (
                "Novelty in the source slate is discounted by this graph's coverage. A missing "
...
class Emission(BaseModel):
    """Everything one slate produced for the valuation stage."""
...
def emit(slate: Slate, frame: ProgramFrame) -> Emission:
    """Group a slate by molecule and emit one `ProgramInput` per molecule."""

    emission = Emission(graph_id=slate.graph_id)
    ...
    for subject, group in by_subject.items():
        ...
        emission.programs.append(program_input(group, slate, frame))
    ...
    if not emission.programs:
        emission.notes.append(
            "No hypothesis in this slate is shaped like a program. LABrador values an "
